...
public:
    ...
    **/
    ...
    @param message_str Resulting message as u8string.
    @param opts        Options to customize formatting.
    @return            Success or error.
    **/
...
protected:
    ...
    **/
    result<std::string> format_subject() const;
    ...
    **/
    ...
    void record_parse_error(error_info err);
...
result_void inline message::add_in_reply_to(const std::string& in_reply)
{
    ...
    if (!detail::is_valid_message_id_strict(id))
        return fail_void(errc::mime_parse_error, "Invalid In Reply To ID.", "In reply to `" + id + "`.");
...
result_void inline message::add_references(const std::string& reference_id)
{
    ...
    if (!detail::is_valid_message_id_strict(id))
        return fail_void(errc::mime_parse_error, "Invalid Reference ID.", "Reference ID is `" + id + "`.");
...
result_void inline message::attachment(std::size_t index, std::ostream& att_strm, string_t& att_name) const
{
    ...
    for (const auto& part : parts_)
    {
        ...
        {
            att_name = part.name();
            const auto& src = part.attachment_source();
            if (src)
            {
                if (src->kind == source_kind::file_path)
                {
                    ...
                    if (!ifs)
                        return fail_void(errc::mime_parse_error, "Attachment read error.",
                    ...
                    while (ifs)
                    {
                        ...
                        auto got = ifs.gcount();
                        if (got > 0)
                            att_strm.write(buffer.data(), got);
                    }
                    return ok();
                }

                att_strm.write(src->bytes.data(), static_cast<std::streamsize>(src->bytes.size()));
                return ok();
            }
...
result_void inline message::parse_header_line(const std::string& header_line)
{
    ...
    else if (detail::iequals_ascii(header_name, CONTENT_ID_HEADER))
    {
        ...
        {
            auto from_res = parse_address_list(header_value);
            if (!from_res)
            {
        ...
        else if (detail::iequals_ascii(header_name, SENDER_HEADER))
        {
            ...
            {
                auto mbx_res = parse_address_list(header_value);
                if (!mbx_res)
                {
        ...
        {
            auto rcpt_res = parse_address_list(header_value);
            if (!rcpt_res)
            {
        ...
        {
            auto cc_res = parse_address_list(header_value);
            if (!cc_res)
            {
        ...
        else if (detail::iequals_ascii(header_name, SUBJECT_HEADER))
        {
            ...
            {
                auto subject_res = parse_subject(header_value);
                if (!subject_res)
                {
        ...
        else if (detail::iequals_ascii(header_name, DATE_HEADER))
        {
            ...
            {
                auto date_res = parse_date(detail::trim_copy(header_value));
                if (!date_res)
                {
...
result<std::string> inline message::format_address_list(const mailboxes& mailbox_list, const std::string& header_name) const
{
    ...
    {
        std::string addr_str;
        MAILXX_TRY_ASSIGN(addr_str, format_address(ma->name, ma->address, header_name));

        if (mailbox_list.addresses.size() > 1 && ma != mailbox_list.addresses.begin())
            mailbox_str += NEW_LINE_INDENT + addr_str;
        else
            mailbox_str += addr_str;
    ...
    for (auto mg = mailbox_list.groups.begin(); mg != mailbox_list.groups.end(); mg++)
    {
        if (!detail::is_group_name(mg->name))
            return fail<std::string>(errc::mime_parse_error, "Address std::list format error.", "Invalid group name `" + mg->name + "`.");

        mailbox_str += mg->name + MAILGROUP_NAME_SEPARATOR + codec::SPACE_CHAR;
        for (auto ma = mg->members.begin(); ma != mg->members.end(); ma++)
        {
            std::string member_str;
            MAILXX_TRY_ASSIGN(member_str, format_address(ma->name, ma->address, header_name));

            if (mg->members.size() > 1 && ma != mg->members.begin())
                mailbox_str += NEW_LINE_INDENT + member_str;
            else
                mailbox_str += member_str;
    ...
    return ok(std::move(mailbox_str));
}


result<std::string> inline message::format_address(const string_t& name, const std::string& address, const std::string& header_name) const
{
    ...
    if (name.codec_type == codec::codec_t::ASCII)
    {
        ...
        if (has_only_tokens)
        {
            ...
            auto enc = b7.encode(name.buffer);
            if (!enc)
                return fail<std::string>(std::move(enc).error());
            name_formatted = std::move(*enc);
        }
        else
        {
            ...
            if (has_only_qtext)
            {
                ...
                auto enc = b7.encode(codec::QUOTE_CHAR + name.buffer + codec::QUOTE_CHAR);
                if (!enc)
                    return fail<std::string>(std::move(enc).error());
                name_formatted = std::move(*enc);
            }
            ...
            {
                return fail<std::string>(errc::mime_parse_error, "Name format error.", "Invalid name is `" + name.buffer + "`.");
            }
        }
    }
    else if (name.codec_type == codec::codec_t::UTF8)
    {
        ...
        auto enc = b8.encode(name.buffer);
        if (!enc)
            return fail<std::string>(std::move(enc).error());
        name_formatted = std::move(*enc);
    }
    else if (name.codec_type == codec::codec_t::BASE64 || name.codec_type == codec::codec_t::QUOTED_PRINTABLE)
    {
        ...
        auto enc = qc.encode(name.buffer, name.charset, name.codec_type);
        if (!enc)
            return fail<std::string>(std::move(enc).error());
        name_formatted = std::move(*enc);
    }
    else if (name.codec_type == codec::codec_t::PERCENT)
        return fail<std::string>(errc::mime_parse_error, "Percent codec not allowed for the mail address.", "");
    ...
    if (!address.empty())
    {
        ...
        else
        {
            ...
            if (has_only_dtext)
                ...
                return fail<std::string>(errc::mime_parse_error, "Address format error.", "Invalid address is `" + address + "`.");
        }
    }
    ...
    return ok(std::move(name_addr));
}
result<std::string> inline message::format_subject() const
{
    std::string subject;
    ...
    if (subject_.codec_type == codec::codec_t::ASCII)
    {
        ...
        auto hdr = b7.encode(subject_.buffer);
        if (!hdr)
            return fail<std::string>(std::move(hdr).error());
        if (hdr->empty())
            return fail<std::string>(errc::mime_parse_error, "Subject format error.", "");
    ...
    else if (subject_.codec_type == codec::codec_t::UTF8)
    {
        ...
        auto hdr = b8.encode(subject_.buffer);
        if (!hdr)
            return fail<std::string>(std::move(hdr).error());
        if (hdr->empty())
            return fail<std::string>(errc::mime_parse_error, "Subject format error.", "");
    ...
    else if (subject_.codec_type == codec::codec_t::QUOTED_PRINTABLE || subject_.codec_type == codec::codec_t::BASE64)
    {
        ...
        auto hdr = qc.encode(subject_.buffer, subject_.charset, subject_.codec_type);
        if (!hdr)
            return fail<std::string>(std::move(hdr).error());
        if (hdr->empty())
            return fail<std::string>(errc::mime_parse_error, "Subject format error.", "");
    ...
    {
        return fail<std::string>(errc::mime_parse_error, "Percent codec not allowed for the subject.", "");
    }
...
*/
result<mailboxes> inline message::parse_address_list(const std::string& address_list)
{
    ...
    auto fail_parse = [&](const std::string& msg, std::string detail) -> result<mailboxes>
    {
        return fail<mailboxes>(errc::mime_parse_error, msg, std::move(detail));
    };
        ...
        switch (state)
        {
            case state_t::BEGIN:
            {
                ...
                else if (std::isalpha(static_cast<unsigned char>(*ch)) || std::isdigit(static_cast<unsigned char>(*ch)) || ATEXT.find(*ch) != std::string::npos || codec::is_8bit_char(*ch))
                {
                    ...
                    return fail_parse("Address or group parsing error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " +
                ...
                if (ch == address_list.end() - 1)
                {
                    ...
                    {
                        if (group_found)
                            return fail_parse("Group parsing error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " +
                                std::to_string(char_pos) + ".\nAddress std::list is `" + address_list + "`.");
                        else
                        {
                            if (!token.empty())
                    ...
                    // `QNAMEADDRBEG` or `ADDRBRBEG`
                        ...
                        return fail_parse("Name or address parsing error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " +
                            std::to_string(char_pos) + ".\nAddress std::list is `" + address_list + "`.");
                }
            ...
            case state_t::NAMEADDRGRP:
            {
                ...
                {
                    if (group_found)
                        return fail_parse("Group parsing error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " + std::to_string(char_pos) +
                            ".\nAddress std::list is `" + address_list + "`.");
                ...
                else if (*ch == ADDRESS_BEGIN_CHAR)
                {
                    cur_address.name = token;
                    detail::trim_inplace(cur_address.name.buffer);
                    MAILXX_TRY_ASSIGN(cur_address.name, parse_address_name(cur_address.name.buffer));
                    token.clear();
                    state = state_t::ADDRBRBEG;
                }
                else
                ...
                if (ch == address_list.end() - 1)
                {
                    if (state == state_t::NAMEADDRGRP)
                    {
                        ...
                        return fail_parse("Address parsing error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " + std::to_string(char_pos)
                        + ".\nAddress std::list is `" + address_list + "`.");
                    else if (state == state_t::NAME)
                        return fail_parse("Name parsing error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " + std::to_string(char_pos) +
                            ".\nAddress std::list is `" + address_list + "`.");
            ...
            case state_t::ADDR:
            {
                ...
                else if (*ch == codec::LEFT_PARENTHESIS_CHAR)
                {
                    ...
                    else
                    {
                        ...
                        cur_address.clear();
                        if (!monkey_found)
                            return fail_parse("Address parsing error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " +
                                std::to_string(char_pos) + ".\nAddress std::list is `" + address_list + "`.");
                        mail_list.insert(mail_list.end(), mail_addrs.begin(), mail_addrs.end());
                    }
                ...
                if (ch == address_list.end() - 1)
                {
                    ...
                    else if (state == state_t::BEGIN)
                    {
                        ...
                        return fail_parse("Comment parsing failure.", "Syntax error at character `" + std::string(1, *ch) + "` at position " +
                            std::to_string(char_pos) + ".\nAddress std::list is `" + address_list + "`.");
                }
            ...
            case state_t::ADDRBREND:
            {
                ...
                {
                    if (group_found)
                        return fail_parse("Comment parsing error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " + std::to_string(char_pos)
                            + ".\nAddress std::list is `" + address_list + "`.");
                    else
                        mail_list.insert(mail_list.end(), mail_addrs.begin(), mail_addrs.end());
                    state = state_t::COMMBEG;
            ...
            case state_t::EOL:
            {
                return fail_parse("Address parsing error.", "Unexpected end-of-line state at position " + std::to_string(char_pos)
                    + ".\nAddress std::list is `" + address_list + "`.");
            }
        }
    }

    return ok(mailboxes(mail_list, mail_group_list));
}
...
*/
result<std::chrono::zoned_time<std::chrono::seconds>> inline message::parse_date(const std::string& date_str) const
{
    auto fail_generic = [&]() -> result<std::chrono::zoned_time<std::chrono::seconds>>
    {
        return fail<std::chrono::zoned_time<std::chrono::seconds>>(errc::mime_parse_error, "Date parsing error.", "Date is `" + date_str + "`.");
    };
    ...
    if (month == 0)
        return fail<std::chrono::zoned_time<std::chrono::seconds>>(errc::mime_parse_error, "Invalid month in date.", "Month is `" + std::string(mon_str) + "`.");
    ...
    try
    {
        return ok(std::chrono::zoned_time<std::chrono::seconds>{std::chrono::current_zone(), sys_tp});
    }
    catch (const std::exception& exc)
    {
        return fail<std::chrono::zoned_time<std::chrono::seconds>>(errc::mime_parse_error,
            "Date parsing error.", exc.what());
    }
    catch (...)
    {
        return fail<std::chrono::zoned_time<std::chrono::seconds>>(errc::mime_parse_error,
            "Date parsing error.", "unknown error");
    }
}
...
inline message::parse_subject(const std::string& subject)
{
    if (codec::is_utf8_string(subject))
        return ok(std::make_tuple(subject, codec::CHARSET_UTF8, codec::codec_t::ASCII));
    ...
    if (!subject_dec)
    {
        std::string detail = subject_dec.error().message;
        if (!subject_dec.error().detail.empty())
        {
            if (!detail.empty())
                detail += ": ";
            detail += subject_dec.error().detail;
        }
        return fail<std::tuple<std::string, std::string, codec::codec_t>>(errc::mime_parse_error,
            "Parsing failure of Q encoding.", std::move(detail));
    }
    return ok(std::make_tuple(std::get<0>(*subject_dec), std::get<1>(*subject_dec), std::get<2>(*subject_dec)));
}


result<string_t> inline message::parse_address_name(const std::string& address_name)
{
    q_codec qc(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
    ...
    if (starts_with_q_encoded_prefix && !ends_with_q_encoded_suffix)
    {
        if (codec::is_utf8_string(address_name))
            return ok(string_t(address_name, codec::CHARSET_UTF8));
        else
            return ok(string_t(address_name, codec::CHARSET_ASCII));
    }
    ...
    if (is_q_encoded)
    {
        ...
        for (const auto& p : parts)
        {
            ...
            if (!an)
            {
                std::string detail = an.error().message;
                if (!an.error().detail.empty())
                {
                    if (!detail.empty())
                        detail += ": ";
                    detail += an.error().detail;
                }
                return fail<string_t>(errc::mime_parse_error, "Parsing failure of Q encoding.", std::move(detail));
            }
            parts_str += std::get<0>(*an);
            if (charset.empty())
                charset = std::get<1>(*an);
            if (charset != std::get<1>(*an))
                return fail<string_t>(errc::mime_parse_error, "Inconsistent Q encodings.", "Charset `" + charset + "` vs charset `" + std::get<1>(*an) + "`.");
            if (!buf_codec)
                buf_codec = std::get<2>(*an);
        }
        if (!buf_codec)
            buf_codec = codec::codec_t::ASCII;
        return ok(string_t(parts_str, charset, buf_codec.value()));
    }
...
void inline message::record_parse_error(error_info err)
{
    if (parse_error_)
        return;
    err.code = errc::mime_parse_error;
    if (err.message.empty())
        err.message = "mime parse error";
    parse_error_ = std::move(err);
}
